...
function ParGeoMap({ geometry }: ParGeoMapProps) {
  ...
  useEffect(() => {
    ...
    const projection = d3.geoAlbers().rotate([67.5, 0]).fitSize([width, height], geometry);
    ...
    const dom_min = d3.min(data, (d: any) => d.detections) || 0;
    const dom_max = d3.max(data, (d: any) => d.detections) || 1;
